...
static void write_logfile(const llama_context* ctx,
...
static void llama_log_callback_logTee(ggml_log_level level,
                                      const char* text,
                                      void* user_data) {
  (void)level;
  (void)user_data;
...
string hchat::getHello() {
  return "Hello, World!";
}

string hchat::getModelPath() {
  string modelDir = "./";
  if (file_exists(modelDir + MODEL)) {
    return modelDir + MODEL;
  }

  modelDir = DEBUG_MODEL_DIR;
  if (file_exists(modelDir + MODEL)) {
    return modelDir + MODEL;
  }
...
int hchat::chat(const string& prompt) {
  gpt_params params;
  g_params = &params;
  params.model = getModelPath();
  ...
  if (ctx_guidance) {
    ...
    guidance_inp = ::llama_tokenize(ctx_guidance, sparams.cfg_negative_prompt,
                                    add_bos, true);
  ...
  if (!session_tokens.empty()) {
    ...
    if (params.prompt.empty() && n_matching_session_tokens == embd_inp.size()) {
      ...
      LOG_TEE(
          "%s: warning: session file has low similarity to prompt (%zu / %zu "
